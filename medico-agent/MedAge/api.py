...
async def chat_upload(
    ...
    files: List[UploadFile] = File(None, description="Upload medical images (optional)")
):
    try:
        ...
        if files:
            ...
            for idx, f in enumerate(files):
                try:
                    ...
                    if filename and filename.strip():  # Skip empty/None filenames
                        valid_files.append(f)
                        logger.info(f"✅ Detected valid file: {filename} (type: {type(f)})")
                    else:
                        logger.warning(f"Skipping file with empty filename at index {idx}: {type(f)}")
                except Exception as e:
                    logger.warning(f"Error checking file {idx} ({type(f)}): {e}")
