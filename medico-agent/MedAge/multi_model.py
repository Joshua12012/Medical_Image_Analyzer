...
"""
Finetuned Multi-model loading and input/answer helpers.
"""
import logging
import os
...
def load_multi_model(model_name: str = "Haider584/lora_model"):
    """
    ...
    if (_model_cache["model"] is None or
        ...
        try:
            ...
            if "unsloth" in str(ie).lower():
                raise ImportError("Unsloth requires a GPU (CUDA). Install torch with CUDA or use CPU fallback.")
            raise
        except NotImplementedError as nie:
            if "accelerator" in str(nie).lower() or "GPU" in str(nie):
                raise RuntimeError("Vision model requires GPU. Run on CPU-only machine? Use text-only fallback.")
            raise
    else:
