...
export default function Sidebar({ chats = [], onSelectChat, onNewChat, selectedId }) {
  ...
  const listVariants = {
    open: {
      opacity: 1,
      height: "auto",
      transition: { when: "beforeChildren", staggerChildren: 0.05, delayChildren: 0.04 },
    },
    closed: {
      opacity: 0,
      height: 0,
  ...
  const itemVariants = {
    ...
    closed: { opacity: 0, y: -10, transition: { duration: 0.18, ease: "easeIn" } },
  };

  return (
    <>
      {/* ---------- Desktop / tablet sidebar (md+) ---------- */}
      <aside className="hidden md:flex w-72 flex-shrink-0 bg-white/80 backdrop-blur-md p-4 flex-col">
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={onNewChat}
            className="p-2 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 text-white shadow-sm hover:scale-[1.03] transition"
        ...
        <AnimatePresence initial={false}>
          {isOpen && (
            ...
            >
              {chats.length === 0 ? (
                <motion.li variants={itemVariants} className="px-2 py-2 text-sm text-slate-400">
                ...
                chats.map((c) => {
                  ...
                  return (
                    <motion.li key={id ?? JSON.stringify(c)} variants={itemVariants}>
                      <button
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-slate-800">Your Chats</h3>
                ...
                <ul className="flex flex-col gap-3">
                  {chats.length === 0 ? (
                    <li className="px-3 py-3 rounded-lg text-sm text-slate-500">No chats yet — start one</li>
                  ) : (
                    chats.map((c) => {
                      const id = getId(c);
                      const isActive = id && selectedId && id === selectedId;
